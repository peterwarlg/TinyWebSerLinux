...
HttpRequestParser::fooParseLine(char *buffer, int &checked_index, int &read_index) {
    char temp;
...
// 分析头部字段
HttpRequestParser::HTTP_CODE
HttpRequestParser::fooParseHeaders(char *line, PARSE_STATE &parse_state, HttpRequest &request) {
...
// 解析body
HttpRequestParser::HTTP_CODE
HttpRequestParser::fooParseBody(char *body, HttpRequest &request) {
...
// http 请求入口
HttpRequestParser::HTTP_CODE
HttpRequestParser::fooParseContent(char *buffer, int &check_index, int &read_index,
                                   HttpRequestParser::PARSE_STATE &parse_state, int &start_line,
                                   HttpRequest &request) {
    LINE_STATE line_state = LINE_OK;
    HTTP_CODE retcode = NO_REQUEST;
    while ((line_state = fooParseLine(buffer, check_index, read_index)) == LINE_OK) {
        char *temp = buffer + start_line;      // 这一行在buffer中的起始位置
        start_line = check_index;              // 下一行起始位置

        switch (parse_state) {
            case PARSE_REQUESTLINE: {
                retcode = fooParseRequestline(temp, parse_state, request);
            ...
            case PARSE_HEADER: {
                retcode = fooParseHeaders(temp, parse_state, request);
                if (retcode == BAD_REQUEST) {
                    return BAD_REQUEST;
            ...
            case PARSE_BODY: {
                retcode = fooParseBody(temp, request);
