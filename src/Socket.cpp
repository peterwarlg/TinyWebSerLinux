//
// Created by peterwarlg on 2021/2/1 4:21 PM.
//
...
void ServerSocket::fooBind() {
    int ret = ::bind(listen_fd, (struct sockaddr*)&mAddr, sizeof(mAddr));
    if (ret == -1) {
        std::cout << "fooBind error in file <" << __FILE__ << "> "<< "at " << __LINE__ << std::endl;
        exit(0);
    }
}

void ServerSocket::fooListen() {
    int ret = ::listen(listen_fd, 1024);
    if (ret == -1) {
        std::cout << "fooListen error in file <" << __FILE__ << "> "<< "at " << __LINE__ << std::endl;
        exit(0);
    }
}

int ServerSocket::fooAccept(ClientSocket &clientSocket) const {
    //std::cout << "listen_fd" << listen_fd << std::endl;
    // FIXME 之前这里出报 Invalid arg
    //int clientfd = ::fooAccept(listen_fd, (struct sockaddr*)&clientSocket.mAddr, &clientSocket.mLen);
    int clientfd = ::accept(listen_fd, NULL, NULL);

    // FIXME 由于之前listen_fd是阻塞的逻辑正确，但是对于非阻塞listen_fd不正确
//    if (clientfd < 0) {
//        std::cout << "fooAccept error in file <" << __FILE__ << "> "<< "at " << __LINE__ << std::endl;
//        exit(0);
//    }
    if (clientfd < 0) {
    ...
    //std::cout << "fooAccept a client： " << clientfd << std::endl;
    clientSocket.fd = clientfd;
    return clientfd;
}

void ServerSocket::fooClose() {
    if (listen_fd >= 0) {
        ::close(listen_fd);
